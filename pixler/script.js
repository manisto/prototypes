...
function generateCanvas() {
    ...
    for (let rowCount = 0; rowCount < data.size; rowCount++) {
        ...
        for (let cellCount = 0; cellCount < data.size; cellCount++) {
            let pixelClicked = function(event) { handlePaint(event, rowCount, cellCount); }
...
function generateStylesheet() {
    let styles = data.colors.map((color, index) => `.c${index} { background-image: none; background-color: ${color}; }`);
    styles.unshift(`table td { background-size: cover; background-image: url('${ generateEmptyBackground() }'); }`);
    ...
    reader.onload = (result) => {
        ...
        for (let currentRow = 0; currentRow < data.size; currentRow++) {
            for (let currentCell = 0; currentCell < data.size; currentCell++) {
                imageBody.children[currentRow].children[currentCell].className = data.pixels[currentRow][currentCell];
            }
        }
    };
